...
onLoad=()=>{
  ...
  fetchApi("/roles/getRights", "GET", {}, header)
  .then(response => {
    ...
    this.setState({
      ...
      var modules = [];
      if(this.state.quObj.rights && this.state.quObj.rights.length){
        this.state.quObj.rights.map((right)=>{
            Object.keys(this.state.data).forEach((module)=>{
              Object.keys(this.state.data[module]).forEach((route)=>{
                if(route === right.path){
                  modules.push(module);
                }
              })
...
// setKeys=(e,key)=>{
//   var permission1 = this.state.permissions;
//   permission1[key]=e.target.checked
//   this.setState({
...
onSave=()=>{
  ...
  let header = {
    ...
    let payload={
      ...
      "roleId":this.state.quObj.roleId
    }
